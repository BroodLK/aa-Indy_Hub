...
def _base_payload(**overrides):
    payload = {
        ...
        "tab": "buy",
        "buy": [34, 35],
        "me_te": {"mainME": 10, "mainTE": 20, "blueprintConfigs": {"500": {"me": 8, "te": 16}}},
...
class ShareStateAllowlistTests(SimpleTestCase):
    def test_round_trip_keeps_safe_scenario_fields(self) -> None:
        ...
        self.assertEqual(state["me_te"]["blueprintConfigs"], {"500": {"me": 8, "te": 16}})
        self.assertEqual(state["prices"][0]["unit_price"], 5.5)
        self.assertEqual(state["shipping"], {"route": 7})
        self.assertEqual(
            {entry["id"] for entry in state["inputs"]}, {"industryFeeEnabledInput", "industryFeeFacilityTaxInput"}
        )
    ...
    def test_private_fields_never_survive(self) -> None:
        ...
        payload["inputs"].append({"id": "industryFeeSystemIdInput", "type": "text", "value": "30000142"})
        payload["inputs"].append({"id": "industryFeeRigIdsInput", "type": "text", "value": "37180"})

        state = decode_share_param(_encode(payload))
        serialized = json.dumps(state)

        for private in ("1035466617946", "30000142", "60003760", "characters", "buybackOrders", "37180"):
    ...
    def test_values_are_bounded_and_normalized(self) -> None:
        state = normalize_share_state(
            _base_payload(
                ...
                tab="run_optimized",
                buy=[34, -1, "x", True],
                me_te={"mainME": 99, "mainTE": -5, "blueprintConfigs": {"abc": {"me": 3}, "7": {"me": 50}}},
                prices=[{"item_type_id": 34, "unit_price": "NaN"}, {"item_type_id": 35, "unit_price": -1}],
...
class ShareSchemaParityTests(SimpleTestCase):
    ...
    def test_input_allowlist_matches_client(self) -> None:
        ...
        block = source.split("const CRAFT_SHARE_INPUT_ALLOWLIST = {", 1)[1].split("};", 1)[0]
        client_ids = {line.strip().split(":", 1)[0] for line in block.splitlines() if ":" in line}
