...
if "eve_sde" in getattr(settings, "INSTALLED_APPS", ()):  # pragma: no branch
    try:  # pragma: no cover - Eve SDE optional
        # Alliance Auth (External Libs)
        from eve_sde.models import ItemType
    except ImportError:  # pragma: no cover - fallback when Eve SDE absent
        ItemType = None
else:  # pragma: no cover - Eve SDE not installed
    ItemType = None
...
def personnal_bp_list(request, scope="character"):
    ...
    try:
        ...
        for bp in bp_items:
            ...
            bp.owner_display = owner_display
            bp.owner_id = owner_id_value
            if is_corporation_scope:
...
def all_bp_list(request):
    ...
    blueprints_page = paginator.get_page(page)
    # Activity options are fixed for manufacturing/reactions in this view.
    raw_activity_options = [(1, "Manufacturing"), (9, "Reactions"), (11, "Reactions")]
    # Apply consistent activity labels
    activity_labels = {
        1: "Manufacturing",
        3: "TE Research",
...
def personnal_job_list(request, scope="character"):
    ...
    try:
        ...
        for job in jobs_on_page:
            ...
            job.research_details = research_details
            ...
            job.copy_details = copy_details

            job.output_name = job.product_type_name or job.product_type_id
...
def craft_bp(request, type_id):
            ...
            **rig,
            ...
            "domains_label": ", ".join(
                CRAFT_RIG_DOMAIN_LABELS.get(
                    str(domain), str(domain).replace("_", " ").title()
                )
    ...
    build_environment = {
        ...
        "structure_material_bonus_pct": round(
            build_structure_material_bonus * 100.0, 3
        ),
    ...
    industry_fee_config = {
        ...
        "rig_ids": _parse_positive_int_list(
            request.GET.get("industry_fee_rig_ids", "")
        ),
    ...
    try:
        ...
        def _resolve_product_domains(product_type_id_value: int | None) -> set[str]:
            ...
            is_component = not is_capital_component and (
                "components" in path_text or "component" in group_name
            )
        ...
        try:
            ...
            logger.exception(
                "Unable to resolve BPC source for user %s", request.user.id
            )
        ...
        try:
            ...
            if bpc_source.get("item_ids") is not None:
                user_blueprints = user_blueprints.filter(
        ...
        if ItemType is not None:
            ...
            try:
                ...
                if market_group_ids:
                    ...
                    parent_ids = {
                        int(g["parent_id"]) for g in market_groups if g.get("parent_id")
                    }
                    if parent_ids:
        ...
        def _market_group_label(it):
            ...
            if market_group_id and market_group_lookup:
                ...
                if group:
                    ...
                    parent_name = ""
                    if parent_id and int(parent_id) in market_group_lookup:
                        parent_name = market_group_lookup[int(parent_id)]["name"]
                    name = group.get("name", "")
                    if parent_name and name:
                        return f"{parent_name} - {name}"
                    return name or parent_name or "Other"
            if getattr(it, "group", None) and getattr(it.group, "name", None):
        ...
        if ItemType is not None:
            for item_type in eve_types:
                ...
                group_name = _market_group_label(item_type)
                ...
                resolved_volume = (
                    packaged_volume if packaged_volume is not None else regular_volume
                )
        ...
        blueprint_payload = {
            ...
            "bp_type_id": type_id,
            "bpc_source": {
                key: value for key, value in bpc_source.items() if key != "item_ids"
            },
            "bp_name": bp_name,
            "name": bp_name,
            "num_runs": num_runs,
