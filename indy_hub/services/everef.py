...
def summarize_job_fees(payload: dict, *, included_sections: set[str] | None = None) -> dict:
    ...
    allowed_sections = {
        str(section_name).strip().lower() for section_name in (included_sections or set()) if str(section_name).strip()
    } or None
