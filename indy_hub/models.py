...
class WeeklyMiningPollRun(models.Model):
    ...
    class Meta:
        ...
        indexes = [
            models.Index(
                fields=["status", "resolve_after"], name="indy_hub_wmpr_resolve_idx"
            ),
            models.Index(
                fields=["config", "created_at"], name="indy_hub_wmpr_cfg_created_idx"
            ),
...
class ProductionSimulation(models.Model):
    ...
    simulation_name_normalized = models.CharField(
    ...
    def save(self, *args, **kwargs):
        self.simulation_name_normalized = self.normalize_simulation_name(
...
class MaterialExchangeConfig(models.Model):
    ...
    ore_refine_rate_percent = models.DecimalField(
        ...
        help_text=_(
            "Effective refine rate (0-100) used when refined-mineral ore pricing is enabled."
        ),
    )
    ...
    def get_buy_market_group_map(self) -> dict[int, list[int] | None]:
        ...
        fallback_groups = self._normalize_structure_ids(
            getattr(self, "allowed_market_groups_buy", None) or []
        )
    ...
    def get_capital_preapproved_state_names(self) -> list[str]:
        raw = list(
            getattr(self, "capital_auto_cancel_preapproved_state_names", []) or []
        )
        normalized: list[str] = []
        for value in raw:
    ...
    def get_capital_auto_cancel_grace_delta(self) -> timedelta:
        ...
        value = max(0, value)
        unit = (
            str(getattr(self, "capital_auto_cancel_delay_unit", "") or "")
            .strip()
            .lower()
        )
    ...
    def get_capital_disabled_ship_groups(self) -> list[str]:
        ...
        for raw_group in raw_groups:
            ...
            group_value = "".join(
                char if (char.isalnum() or char == "_") else "_" for char in group_value
            ).strip("_")
            if not group_value:
                continue
...
class MaterialExchangeItemPriceOverride(models.Model):
    ...
    sell_markup_base_override = models.CharField(
        ...
        blank=True,
        help_text=_(
            "Override base price for sell side markup (Jita Buy or Jita Sell)."
        ),
    )
...
class MaterialExchangeSellOrder(models.Model):
    ...
    status_before_rejection = models.CharField(
        ...
        blank=True,
        help_text=_(
            "Previous status restored when an accidental rejection is reopened."
        ),
    )
...
class CapitalShipOrderEvent(models.Model):
    class EventType(models.TextChoices):
        ...
        OFFER_CONFIRMED_BY_USER = "offer_confirmed_by_user", _(
            "Offer confirmed by user"
        )
...
class ReprocessingServiceRequestOutput(models.Model):
    ...
    def __str__(self):
        return (
            f"{self.type_name or self.type_id}: " f"{self.expected_quantity} expected"
        )
...
class PublicJitaContractItem(models.Model):
    ...
    def __str__(self):
        return (
            f"Public contract {self.contract_id} item {self.type_id} x{self.quantity}"
        )
...
class CompressedOreCache(models.Model):
    ...
    CACHE_VERSION = (
        5  # Incremented to force rebuild after excluding batch compressed ore variants
    )
